...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    source_id_text = [[source_vocab_to_int[word] for word in sentence.split(' ') if word] for sentence in source_text.split('\n')]
    target_id_text = [[target_vocab_to_int[word] for word in sentence.split(' ') if word] for sentence in target_text.split('\n')]
    for target_id_sentence in target_id_text:
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    target_end = tf.strided_slice(target_data,[0,0],[batch_size,-1],[1,1])
    target_front = tf.fill([batch_size,1],target_vocab_to_int['<GO>'])
    process_decoding_input = tf.concat([target_front, target_end],1)
        
    return process_decoding_input
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    _, enc_state = tf.nn.dynamic_rnn(lstm_mul, rnn_inputs, dtype=tf.float32)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    # 3. decoder input
    target_decoder_input=process_decoding_input(target_data, target_vocab_to_int, batch_size)
    
    # 4. Decoder Embedding
    dec_embeddings = tf.Variable(tf.random_uniform([target_vocab_size, dec_embedding_size]))
    dec_embed_input = tf.nn.embedding_lookup(dec_embeddings, target_decoder_input)
    
    # 5. Decoder Embedding
    decoder_layer=decoding_layer(dec_embed_input, dec_embeddings, encoder_state, target_vocab_size, 
